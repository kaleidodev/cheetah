...
function ToolbarInput({ placeholder }: { placeholder: string }) {
  return (
    <Input
      ...
      className="h-10 rounded border border-[#c7ced2] bg-[#f3f5f6] px-3 text-[12px] font-medium text-[#6f7c83] placeholder:text-[#a5afb5] shadow-none focus-visible:ring-0"
    />
  );
}
...
function ToolbarSelect({ label, wide }: { label: string; wide?: boolean }) {
  return (
    <button
      type="button"
      className={`inline-flex h-10 items-center justify-between rounded border border-[#a8b1b6] bg-white px-3 text-[12px] font-semibold text-[#458479] ${wide ? "min-w-[170px]" : "min-w-[122px]"} hover:bg-[#f8fafb]`}
    >
      <span>{label}</span>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Select value={offerType} onValueChange={setOfferType}>
            <SelectTrigger className="h-10! min-w-[120px] rounded border border-[#c7ced2] bg-[#f3f5f6] px-3 text-[12px] font-medium text-[#a5afb5] shadow-none focus:ring-0 focus:ring-offset-0 focus-visible:ring-0 data-placeholder:text-[#a5afb5] data-[state=open]:border-[#5ca5a4] data-[state=open]:bg-[#def2ef]">
            ...
            >
              {offerTypeOptions.map((option) => (
                <SelectItem
                  key={option.value}
                  value={option.value}
                  className="cursor-pointer rounded-none px-3 py-1.5 text-[12px] text-[#5f6d75] focus:bg-[#def2ef] focus:text-[#4e7981] data-[state=checked]:bg-[#def2ef] data-[state=checked]:text-[#4e7981]"
                >
                  {option.label}
                </SelectItem>
              ))}
          ...
          <Button
            ...
            className={`h-10 min-h-10 rounded border px-3 text-[12px] font-semibold leading-none transition-colors ${
              createOfferActive
      ...
      <div className="mb-5 flex items-center">
        <Tabs value={layout} onValueChange={(value) => setLayout(value as "grid" | "list")}>
          <TabsList className="h-auto gap-0 bg-transparent p-0">
            <TabsTrigger
              value="grid"
              className="inline-flex h-10 items-center rounded-l-lg rounded-r-none border border-[#d2d8dc] bg-[#f2f4f5] px-2 text-[12px] font-semibold tracking-tight text-[#aeb7bc] data-active:border-2 data-active:border-[#2ea6a9] data-active:bg-[#dff1f2] data-active:text-[#3a737d] data-active:shadow-none after:hidden"
            >
            ...
            <TabsTrigger
              value="list"
              className="inline-flex h-10 items-center rounded-r-lg rounded-l-none border border-[#d2d8dc] bg-[#f2f4f5] px-2 text-[12px] font-semibold tracking-tight text-[#aeb7bc] data-active:border-2 data-active:border-[#2ea6a9] data-active:bg-[#dff1f2] data-active:text-[#3a737d] data-active:shadow-none after:hidden"
            >
